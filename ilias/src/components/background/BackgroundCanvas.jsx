...
export const BackgroundCanvas = ({ scrollRef }) => {
    ...
    const getFilters = () => {
        console.log(offset)
        if (offset < 50) return { brightnessTop: 1.5, opacity: 1, brightnessBot: .4, contrast: 1.4, hue: 0, opacityStars: 0 }
        if (offset < 1500) return { brightnessTop: 1.4, opacity: 1, brightnessBot: .9, contrast: 1.4, hue: 0, opacityStars: 0 }
        if (offset < 2500) return { brightnessTop: .5, opacity: .9, brightnessBot: .9, contrast: 1.4, hue: -60, opacityStars: 0 }
        else return { brightnessTop: .5, opacity: 0, brightnessBot: .5, contrast: .9, hue: -180, opacityStars: 1 }
    }
    ...
    return (
        <>
            <div style={{ position: "relative", zIndex:0 }}>
                <div style={{ zIndex: 1, opacity: filters.opacityStars, pointerEvents:"none", position:"absolute", transition:"opacity 1s 1s" }}>
                ...
                <div><img className="skyPicture" src={DawnImg.src} style={{ height: "100vh", width: "100vw", position: "absolute", filter: `brightness(${filters?.brightnessBot}) contrast(${filters?.contrast}) hue-rotate(${filters?.hue}deg)`, left: 0, top: 0 }} ></img></div>
                <div className="skyPicture" style={{ background: `url(${BlueImg.src}) no-repeat`, backgroundSize: "100%", height: "100vh", width: "100vw", filter: `opacity(${filters?.opacity}) brightness(${filters?.brightnessTop})`, position: "absolute", left: 0, top: 0, mixBlendMode: "screen" }} ></div>
